...
impl Solution {
    pub fn reorganize_string(s: String) -> String {
        ...
        char_array.iter().for_each(|&x| *char_map.entry(x).or_insert(0) += 1);
        char_map.into_iter().for_each(|(letter, count)| queue.push(Item::new(count, letter)));
...
mod test {
    ...
    #[test]
    fn test_one() {
        assert_eq!("aba".to_string(), Solution::reorganize_string("aab".to_string()));
    }

    #[test]
    fn test_two() {
        assert_eq!(String::new(), Solution::reorganize_string("aaab".to_string()));
    }
}