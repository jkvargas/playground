        ...
        if intervals.len() > 1 {
            for pos in 1..intervals.len() {
                ...
                if Self::overlaps(&in_stack, &intervals[pos]) {
                    result.push(vec![in_stack[0], std::cmp::max(in_stack[1], intervals[pos][1])]);
                } else {
...
mod tests {
    ...
    #[test]
    fn merge_2() {
        assert_eq!(Solution::merge(vec![vec![1, 7], vec![4, 5], vec![12, 14], vec![18, 19]]), vec![vec![1, 7], vec![12, 14], vec![18, 19]]);
    }
    ...
    #[test]
    fn merge_4() {
        assert_eq!(Solution::merge(vec![vec![4, 9], vec![4, 5], vec![5, 12], vec![15, 21]]), vec![vec![4, 12], vec![15, 21]]);
    }
