    ...
    pub fn search_range(nums: Vec<i32>, target: i32) -> Vec<i32> {
        ...
        match Self::try_binary_search(&nums, 0, nums.len() as i32 - 1, target, &mut ignore) {
            ...
            Some(mut last_found) => {
                ...
                while let Some(new_search) =
                    Self::try_binary_search(&nums, last_found + 1, end as i32, target, &mut ignore)
                {
                    last_found = new_search;
                }

                while let Some(new_search) =
                    Self::try_binary_search(&nums, 0, first_found - 1, target, &mut ignore)
                {
