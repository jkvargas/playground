...
impl SolutionJhonny {
    pub fn can_cross(stones: Vec<i32>) -> bool {
        ...
        for (i, &val) in stones.iter().enumerate() {
            map.insert(val, i);
        }

        Self::try_jump(0, &map, &stones, 1, &mut set)
    }

    fn get_hash(ind: usize, k: i32) -> u64 {
        (ind as u64 * 3) + (k as u64 * 5)
    }

    fn try_jump(
        ind: usize,
        stones: &HashMap<i32, usize>,
        vec: &Vec<i32>,
        k: i32,
        map: &mut HashSet<u64>,
    ) -> bool {
        let actual_position = vec[ind] + k;
        let hash = Self::get_hash(ind, k);

        if map.contains(&hash) {
            return false;
        } else {
            if let Some(&actual_pos_exist) = stones.get(&actual_position) {
                let final_pos = vec.len() - 1;
                if actual_pos_exist == final_pos {
                    return true;
                } else {
                    let init = k - 1;
                    let range = if init < 1 { k } else { init }..=k + 1;
                    for i in range {
                        println!("act pos: {}, i: {}", actual_position, i);
                        if Self::try_jump(actual_pos_exist, stones, vec, i, map) {
                            return true;
                        }
                    }
                }
...
impl Solution {
    pub fn can_cross(stones: Vec<i32>) -> bool {
        ...
        if let Some(m) = map.get_mut(&0) {
            m.insert(0);
        }

        for i in 0..stones.len() {
            for k in map.get(&stones[i]).unwrap().clone() {
                for step in k - 1..=k + 1 {
                    let n_step = stones[i] + step;
                    if step > 0 {
                        if let Some(m) = map.get_mut(&n_step) {
                            m.insert(step);
                        }
                    }
                }
        ...
        map.get(&stones[stones.len() - 1]).unwrap().len() > 0
    }
}
...
fn testthree() {
    assert!(Solution::can_cross(vec![
        ...
        995, 996, 997, 998, 1035
    ]));
}

#[test]
fn testsec() {
    assert_eq!(false, Solution::can_cross(vec![0, 1, 2, 3, 4, 8, 9, 11]));
}
